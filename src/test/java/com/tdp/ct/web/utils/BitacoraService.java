...
public class BitacoraService {
    ...
    private String[] crearDatosBitacora(String orden) {
        return new String[]{
                getToday(),
                calcularTipoVentana(),
                (String) getScenarioContext().get("analistaQa"),
                getScenarioContext().get("hu") + "/" + getScenarioContext().get("test"),
                (String) getScenarioContext().get("nroDocumento"),
                (String) getScenarioContext().get("tipoDocumento"),
                segment,
                (String) getScenarioContext().get("transaccion"),
                (String) getScenarioContext().get("tipoVenta"),
                aplication,
                (String) getScenarioContext().get("idCliente"),
                (String) getScenarioContext().get("usuarioVendedor"),
                orden.trim(),
                (String) getScenarioContext().get("numeroSolicitud"),
                (String) getScenarioContext().get("codigoAutogestion"),
                (String) getScenarioContext().get("codigoReclamo"),
                (String) getScenarioContext().get("simcard"),
                (String) getScenarioContext().get("imei"),
                (String) getScenarioContext().get("nroLinea"),
                (String) getScenarioContext().get("estadoOrden"),
                getHostname()
        };
    }
}
