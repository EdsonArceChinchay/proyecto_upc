...
import static com.tdp.ct.web.utils.FileUtils.cleanFile;
...
public class CaptchaPage extends WebBase {
    ...
    public void getCaptcha() {
        ...
        UtilWeb.logger(this.getClass()).log(Level.INFO, String.format("captchaElement displayed... %s", captchaElement.isDisplayed()));
        int retries = 0;
        while (!captchaElement.isDisplayed() && retries < MAX_RETRIES) {
            updateCaptcha();
            UtilWeb.waitForSeconds(retries);
            UtilWeb.logger(this.getClass()).log(Level.INFO, String.format("captchaElement displayed... %s - retries: %s.", captchaElement.isDisplayed(), (retries + 1)));
            retries++;
        }
    ...
    private void saveCaptchaScreenshot(WebElement captchaElement) {
        try {
            ...
            UtilWeb.logger(this.getClass()).log(Level.INFO, String.format("Screenshot saved in: %s.", path));
            decodeCaptcha(path);
        } catch (Exception e) {
            UtilWeb.logger(this.getClass()).log(Level.SEVERE, String.format("Error saving screenshot: %s.", e.getMessage()));
        }
    }
    ...
    private void handleCaptchaResult(ImageToText api) {
        ...
        if (captchaText.trim().length() == 4) {
            typeCaptcha(captchaText);
        } else {
            UtilWeb.logger(this.getClass()).log(Level.INFO, String.format("Captcha length is less than 4... has %d digits", captchaText.length()));
            getCaptcha();
        }
    }
